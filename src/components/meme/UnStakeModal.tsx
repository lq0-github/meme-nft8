...
function UnStakeModal(props: any) {
  ...
  const disabled = useMemo(() => {
    if (selectedTab === "meme") {
      return Big(amount || 0).lte(0) || Big(amount || 0).gt(stakedBalance);
    } else {
      const minDeposit = toReadableNumber(
        xrefSeeds[MEME_TOKEN_XREF_MAP[seed_id]].seed_decimal,
        xrefSeeds[MEME_TOKEN_XREF_MAP[seed_id]].min_deposit
      );
      return (
        Big(xrefAmount || 0).lte(0) ||
        Big(xrefAmount || 0).gt(xrefStakedBalance) ||
        Big(xrefAmount || 0).lt(minDeposit)
      );
    }
  }, [
    selectedTab,
