...
const Countdown: React.FC<CountdownProps> = ({ onCountdownFinish }) => {
  ...
  useEffect(() => {
    const targetDate = new Date(
      Date.UTC(new Date().getUTCFullYear(), 11, 1, 0, 0, 0) // 12月1日
    );
    const endDate = new Date(
      Date.UTC(new Date().getUTCFullYear(), 11, 7, 0, 0, 0) // 12月7日
    );
  ...
  return (
    <>
      {countdownFinished ? (
        ...
        <div className="mt-12 bg-dark-10 border border-dark-100 rounded-2xl h-72 relative xsm:mx-4 xsm:h-full xsm:p-4 xsm:pb-10">
