...
function VoteModel(props: any) {
  ...
  const minStakeAmount = toReadableNumber(
    xrefSeeds?.[MEME_TOKEN_XREF_MAP[selectedTab]]?.seed_decimal,
    xrefSeeds?.[MEME_TOKEN_XREF_MAP[selectedTab]]?.min_deposit || "0"
  );
  const disabled =
    Big(amount || 0).lte(0) ||
    ...
    Big(amount || 0).lt(minStakeAmount);
  ...
  console.log(
    xrefSeeds?.[MEME_TOKEN_XREF_MAP[selectedTab]],
    "xrefSeeds?.[MEME_TOKEN_XREF_MAP[selectedTab]]"
  );
  ...
  ${toReadableNumber(
    xrefSeeds?.[MEME_TOKEN_XREF_MAP[selectedTab]]?.seed_decimal,
    xrefSeeds?.[MEME_TOKEN_XREF_MAP[selectedTab]]?.min_deposit
  )}
  xREF, which will be locked during the MEME season.
